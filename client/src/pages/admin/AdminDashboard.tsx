...
export default function AdminDashboard() {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          "Failed to reteive Adminastrator Data",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleApproveStatus = async (
    ...
    try {
      ...
      toast.success(`Restaurent has been Marked as ${status.toUpperCase()}`);

      //Reload local lists and Stats
      const rRes = await await api.get("/admin/restaurants");
      setRestaurants(rRes.data);
